$(document).on('turbolinks:load',function(){
  $(function(){
    ...
    $(".card-icons__previous").on("click", function(){
      var this_page_move_forms = $(".task-box-wrapper__contentent__cards")
      this_page_move_forms.sort(function(a,b){
      ...
      $.each(this_page_move_forms, function(index, a_form){
        var each_list_id = $(a_form).data("id");
        if(each_list_id <  this_form_data_id){
          sessionStorage.setItem("list_id_previous", each_list_id)
          return false
        };
      });
      var list_id_previous = Number(sessionStorage.getItem("list_id_previous"))
      var $dir = location.href.split("/");  
      var $dir2 = $dir[$dir.length -1];
